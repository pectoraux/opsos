/**
 * @kernel/organizations/application/command-context — local CommandContext.
 *
 * The minimal context every organizations command handler needs: a clock, a
 * seeded random source, the acting principal/tenant (opaque, may be null for
 * bootstrap commands like `createOrganization`), and a correlation id for
 * event metadata threading.
 *
 * Determinism rules:
 ...
 * needed. Organizations deliberately does NOT import `@kernel/runtime` (the
 * dependency graph forbids it — organizations depends only on shared-kernel,
 * events, observability, and identity *interface only*); structural
 * compatibility is the seam.
 */
